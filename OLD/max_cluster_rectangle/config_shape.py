...
def seq(length,breadth):
     # Input: Max cluster rectngle dimensions.
     # Output: List of all allowed spanning rectangles.
    shapes = []
    for l in range(1 , length + 1):
        for b in range(1 , breadth + 1):
            for wastage in range(0 , (l * b) - (l + b - 1) + 1):
...
def show_configs_of_size(length,breadth):
     # Input: Max family Size
     # Output: Prints all allowed configurations.
     total = 0
     con = []
     shapes = seq(length,breadth)
     for i in shapes:
        #print(i)
        a = i[0]*i[1]
        ar = [[ 1 for x in range(i[1])] for y in range(i[0])]
...
def plot_config():
     # Input: Max cluster rectangle dimensions.
     # Output: Number of configs vs family size graph
     length = int(input("Enter length of max cluster rectangle: "))
     breadth = int(input("Enter breadth of max cluster rectangle: "))
     numcon = [0 for i in range(length*breadth)]
     shapes = seq(length,breadth)
     x_list = [(i+1) for i in range(length*breadth)]
     for i in shapes:
        print(i)
        a = i[0]*i[1]
        ar = [[ 1 for x in range(i[1])] for y in range(i[0])]
    ...
    #print(con)
     print(numcon)
     plt.title("Number of configurations in a ("+ str(length)+','+str(breadth)+")" + ' rectangle')
     plt.xlabel("Size of configurations")
     plt.ylabel("Number of configurations")
     plt.plot(x_list,numcon,marker='X')
